...
const FranchiseDetailPage = () => {
    ...
    const { data: franchiseDetailsData, isLoading: franchiseDetailsLoading, isError: franchiseDetailsErr } = useFranchiseDetailsQuery({ guid });

    return (
        <>
            <Meta title={franchiseDetailsData?.name === undefined ? "Welcome to GMDB" : `${franchiseDetailsData?.name} - GMDB`} />

            {
                franchiseDetailsErr ? <UnauthorizedPage /> : (
